...
def plot_shap_values(model, explainer, shap_values_cat_train, shap_values_cat_test, customer_id, X_test, X_train):
    ...
    shap.decision_plot(explainer.expected_value, shap_values_cat_test[customer_index],
                       X_test[X_test['customerID'] == customer_id], link="logit")
    st.pyplot(fig)
    plt.close()
...
def main():
    ...
    if election == "User-based SHAP":
        ...
        st.write(
            f"Customer {customer_id}: CatBoost Model's prediction for the Customer Churn : {y_pred[customer_index]}")
    ...
    # If Calculate CHURN Probability option is selected
        ...
        if confirmation_button:
            ...
            new_customer_data_processed = pd.get_dummies(new_customer_data)

            # Ensuring all necessary columns are present
            missing_cols = set(X_train.columns) - set(new_customer_data_processed.columns)
            for col in missing_cols:
                new_customer_data_processed[col] = 0
            new_customer_data_processed = new_customer_data_processed[X_train.columns]
            prediction = model.predict(new_customer_data_processed)
            st.write(f"CHURN probability: {prediction[0]}")
